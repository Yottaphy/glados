...
class Nucleus:
    def __init__(self, n, z, alphaE, alphaEerr):
        self.n           = int(n)
        self.z           = int(z)
        self.a           = int(n+z)
        self.alpha       = alphaE
        self.delta_alpha = alphaEerr         

    def display(self, superscript = False):
        symbol = elements[self.z]
        if superscript:
            a      = makeSuperscript(self.a)
            print(a+symbol)
        else:
            print(symbol+str(self.a))
...
def alphaDaughter(parent):
    return dicNuc[elements[parent.z-2] + str(parent.a-4)]


if __name__ == '__main__':
    _, n, z, e, d = np.genfromtxt( "alphas.txt", unpack=True, skip_header=1, skip_footer=0, autostrip=True)

    dicNuc = {}
    for i in range(0,6655):
        if n[i] > 126 and n[i] < 156 and z[i] > 83 and z[i] < 103:
            nuc = Nucleus(n[i], z[i], e[i], d[i])             
            dicNuc[elements[nuc.z] + str(nuc.a)] = nuc

    alphaDaughter(dicNuc['U235']).display()
